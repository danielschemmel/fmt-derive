...
pub fn display(item: proc_macro::TokenStream, lib: &proc_macro2::TokenStream) -> proc_macro::TokenStream {
...
fn process_tuple(
	...
	for (field_number, field) in fields.unnamed.iter().enumerate() {
		...
		if config.ignore {
			...
			chain.extend(quote!(#lib::_rt::DisplayOrReplacement::<#field_type>(&#var_name).tuple_field(#opaque, &mut w);));
		}
	}
...
fn process_struct(
	...
	for field in &fields.named {
		...
		if config.ignore {
			...
			let opaque = opaque_object_string(field_type);
			chain.extend(
				quote!(#lib::_rt::DisplayOrReplacement::<#field_type>(&#field_name).struct_field(#field_name_str, #opaque, &mut w);),
			);
		}
	}
